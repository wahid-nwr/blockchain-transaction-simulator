...
describe("TransferService", () => {
    ...
    tokenRepositoryMock.findById
        .mockResolvedValue({
            id: "token-1",
            contractAddress: "0x5FbDB2315678afecb367f032d93F642f64180aa3"
        });

    walletRepositoryMock.findById
    ...
    expect(tokenRepositoryMock.findById)
        .toHaveBeenCalledWith(
            "token-1"
        );

    expect(walletRepositoryMock.findById)
        .toHaveBeenCalled();

    expect(ledgerMock.createPending)
        .toHaveBeenCalled();

    expect(walletClient.writeContract)
        .toHaveBeenCalled();

    expect(ledgerMock.attachHash)
        .toHaveBeenCalledWith(
            "tx-123",
            "0xtransactionhash"
        );
